...
namespace Soko.Common.Controls.Gauges
{
    public class LinearGauge : Control
    {
      ...
      Font majorScaleFont = new Font(this.Font.FontFamily, mMajorScaleHeight, this.Font.Style);
            ...
            graphics.DrawRectangle(majorGridPen, mGaugeRegion.Left, mGaugeRegion.Top,
                mGaugeRegion.Width, mGaugeRegion.Height);
            majorGridPen.Alignment = PenAlignment.Center;
            ...
            for (int i = 0; i <= MajorGrid; ++i)
            {
                ...
                float fadeOut = 4.0f * (mMinorScaleHeight + mMajorScaleHeight) / (majorGridStep / MinorGrid) - 2.0f;
                //fadeOut = fadeOut * fadeOut;

                //graphics.DrawString(fadeOut.ToString(), majorScaleFont, scaleBrush,
                //    60, majorGridPosition, scaleLabelFormat);
                ...
                if (i == MajorGrid - 1)
                    labelBottomMargin += mMajorScaleHeight * 0.5f * fadeOut;
                graphics.DrawString(Math.Round(GetValueAtPosition(majorGridPosition), 0).ToString(), majorScaleFont, scaleBrush,
                    mScaleRegion.X + mScaleRegion.Width * 0.5f, labelPosition, scaleLabelFormat);
                ...
                if (i < MajorGrid)
                {
                    ...
                    for (int j = 1; j < MinorGrid; ++j)
                    {
                        ...
                        graphics.DrawString(Math.Round(GetValueAtPosition(minorGridPosition), 0).ToString(), minorScaleFont, scaleBrush,
                            mScaleRegion.X + mScaleRegion.Width * 0.5f, minorGridLabelPosition, scaleLabelFormat);
                    }
                }
            }
      ...
      Font valueLabelFont = new Font(this.Font.FontFamily, mValueHeight, this.Font.Style);
            ...
            {
                Brush valueBrush = new LinearGradientBrush(valueRect, Color.Green, Color.Lime, 0.0f, false);
        ...
        */
        ...
        //private float GetValueAtPosition(float position)
        //{
